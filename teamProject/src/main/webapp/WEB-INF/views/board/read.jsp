...
<script>
	...
	$("#a_list").click(
			function(e) {
				e.preventDefualt;
				frmPaging.find("input[name=bno]").val(
						"${boardVo.bno}");
...
<div class="container-fluid">
		...
		<div class="col-md-12">
			<form class="role-form" action="/board/updateRun" method="post">
				<input type="hidden" name="bno" value="${boardVo.bno}"> <input
					...
					type="hidden" name="perPage" value="${pagingDto.perPage}">
				<input type="hidden" name="searchType"
					value="${pagingDto.searchType}"> <input type="hidden"
					name="keyword" value="${pagingDto.keyword}">

				<div class="jumbotron">
					<h2>글 내용 보기</h2>
					<p>
						<a class="btn btn-primary btn-large" id="a_list"
				...
				<c:choose>
					<c:when test="${loginVo.member_id == boardVo.writer}">
						...
						<button type="submit" class="btn btn-success" id="btnUpdateRun"
							style="display: none;">수정완료</button>
						<a class="btn btn-danger" id="btnDelete" href="${boardVo.bno}">삭제</a>
					</c:when>
				</c:choose>
				<a class="btn btn-warning"
					href="/board/replyForm?bno=${boardVo.bno}">답글</a>
	...
	<div class="row" style="margin-top: 30px;">
		...
		<div class="col-md-2">
			<input type="text" class="form-control" id="c_member_id"
				value="${loginVo.member_id}" readonly="readonly">
		</div>
		<div class="col-md-1">
			...
			<table style="display: none;" id="table_clone">
				<tr>
					...
					<td>
						...
						<button type="button"
							class="btn btn-sm btn-success btnCommentModifyFinish"
							style="display: none;">수정완료</button>
					</td>
					<td>
						<button type="button"
